public class MyLinkedList<E extends Comparable<E>>  
{
  ...
  public E getMin() 
  {
	  //Sees if the list is empty and then also makes the value null
        if (getFirst() == null) 
		{
            return null;
        }
		
		//Gets the first element of the list and the fisrt node
        E minimum = getFirst(); 
        Node<E> immediate = head; 
		E thisElement = immediate.element;

        while (immediate != null) 
		{
            if (thisElement.compareTo(minimum) < 0) 
			{
				//Makes a new minimum if its smaller the surrennt one is smaller than the previous
                minimum = thisElement; 
            }
